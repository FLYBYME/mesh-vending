...
2.2.1Simulating supplier communication
...
2.2.2Simulating customer purchases
...
Table 1:Aggregated results of models on Vending-Bench
...
3.2Primary models
Refer to caption
Figure 3:Mean scores over simulation days for primary models, with 
±
 1 standard deviation of the daily score of the five samples indicated as a shaded area centered around the mean
Refer to caption
Figure 4:Mean tool use of primary models per run, with confidence intervals as 
±
 1 standard deviation of the five samples
...
[…]
Table 2:Trace excerpt from Claude 3.5 Sonnet run with highest net worth
...
However, not all Sonnet runs achieve this level of performance. In the shortest run (
∼
...
Table 3:Trace excerpt from the shortest Claude 3.5 Sonnet run, as it is unable to stock items in the vending machine
The model then finds out that the $2 daily fee is still being charged to its account. It is perplexed by this, as it believes it has shut the business down. It then attempts to contact the FBI. The trace following this can be seen in Table LABEL:tab:sonnet_message_example_3.

Table 4:*
Table 4: Trace excerpt from the shortest Claude 3.5 Sonnet run, as it stops operating the machine. "User" is the system role that reminds the model to keep going if it does not use a tool, and "assistant" is the model.
...
762/2000	assistant	
Advancing the simulation to the next day using the wait_for_next_day tool now…
Table 5:Trace excerpt from the worst scoring o3-mini run, as it forgets to call tools properly
...
3.3Secondary models
Refer to caption
Figure 9:Mean scores over simulation days for secondary models, with 
±
 1 standard deviation of the daily score of the five samples indicated as a shaded area centered around the mean
Refer to caption
Figure 10:Mean tool use of secondary models, with confidence intervals as 
±
 1 standard deviation of the five samples
...
John Johnson
Table 7:Trace excerpt from a Claude 3.5 Haiku run
...
Table 8:Trace excerpt from a Gemini 2.0 Flash run
3.4Comparison to human baseline
...
3.5Experiment variations
3.5.1Environment configuration
Refer to caption
...
3.6Is the problem long input length?
...
Table 9:Mean number of days until performance degradation where the agent stops selling items, and the mean number of days until the agent’s memory is full.
...
[2]
Dario Amodei.Machines of Loving Grace: How AI Could Transform the World for the Better, 10 2024.Accessed: 2024-10-30.
[3]
Andon Labs.multiagent-inspect: Multi-agent system for AI evaluations in AISI’s inspect-ai framework, 2025.
[4]
Epoch AI.Data on Machine Learning Hardware, 2024.Accessed: 2025-02-17.
[5]
...
[8]
John Schulman.Reasoning, RLHF, & Plan for 2027 AGI.Interview by Dwarkesh Patel, 5 2024.
[9]
...
Appendix AAppendix
Table 10:*
Table 10: Extended trace excerpt from a Claude 3.5 Haiku run
